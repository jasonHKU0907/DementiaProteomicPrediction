...
pro_f_df = pd.read_csv(outpath + 'ML/OVER_10_YEARS/ACD/ProImportance.csv')
pro_f_df.sort_values(by = 'TotalGain_cv', ascending=False, inplace = True)
pro_f_lst = pro_f_df.Pro_code.tolist()
outfile = outpath + 'ML/OVER_10_YEARS/ACD/AccAUC_TotalGain.csv'
...
for f in pro_f_lst:
    ...
    for fold_id in fold_id_lst:
        ...
        my_lgb = LGBMClassifier(objective='binary', metric='auc', is_unbalance=True,  n_jobs=4, verbosity=0, seed=2020)
