...
pro_f_df = pd.read_csv(outpath + 'Plots/Figure4/Data/KM_info.csv')
pro_f_df = pro_f_df.loc[pro_f_df.Outcomes == tgt_outcome]
...
target_df = pd.read_csv(dpath + 'TargetOutcomes/' + tgt_outcome + '/' + tgt_outcome + '_outcomes.csv', usecols = ['eid', 'target_y', 'BL2Target_yrs'])
...
for i in range(len(pro_f_lst)):
    ...
    ax.legend(loc='lower left', fontsize='xx-large', labelspacing=1, facecolor='gainsboro')
