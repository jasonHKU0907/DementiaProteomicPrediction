...
dpath = '/Volumes/JasonWork/Projects/AD_Proteomics/Data/TargetOutcomes/ACD/'
alg_df = pd.read_csv(dpath + 'ACD_ALG.csv', usecols = ['eid', 'target_y', 'BL2Target_yrs', 'Target_source'])
alg_df.rename(columns = {'target_y':'alg_y', 'BL2Target_yrs':'alg_yrs', 'Target_source':'alg_source'}, inplace = True)

fo_df = pd.read_csv(dpath + 'ACD_FO.csv', usecols = ['eid', 'target_y', 'BL2Target_yrs', 'Target_source'])
fo_df.rename(columns = {'target_y':'fo_y', 'BL2Target_yrs':'fo_yrs', 'Target_source':'fo_source'}, inplace = True)

death_df = pd.read_csv(dpath + 'ACD_Death.csv', usecols = ['eid', 'target_y', 'BL2Target_yrs'])
death_df.rename(columns = {'target_y':'death_y', 'BL2Target_yrs':'death_yrs'}, inplace = True)

inhosp_df = pd.read_csv(dpath + 'ACD_InHosp.csv', usecols = ['eid', 'target_y', 'BL2Target_yrs'])
...
mydf.to_csv(dpath + 'ACD_outcomes.csv', index = False)
