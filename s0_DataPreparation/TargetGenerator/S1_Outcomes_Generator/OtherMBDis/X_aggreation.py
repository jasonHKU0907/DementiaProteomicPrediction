...
dpath = '/Volumes/JasonWork/Projects/AD_Proteomics/Data/TargetOutcomes/OtherMBDis/'

fo_df = pd.read_csv(dpath + 'OMBD_FO.csv', usecols = ['eid', 'target_y', 'BL2Target_yrs', 'Target_source'])
fo_df.rename(columns = {'target_y':'fo_y', 'BL2Target_yrs':'fo_yrs', 'Target_source':'fo_source'}, inplace = True)

death_df = pd.read_csv(dpath + 'OMBD_Death.csv', usecols = ['eid', 'target_y', 'BL2Target_yrs'])
death_df.rename(columns = {'target_y':'death_y', 'BL2Target_yrs':'death_yrs'}, inplace = True)

inhosp_df = pd.read_csv(dpath + 'OMBD_InHosp.csv', usecols = ['eid', 'target_y', 'BL2Target_yrs'])
...
mydf.to_csv(dpath + 'OMBD_outcomes.csv', index = False)
