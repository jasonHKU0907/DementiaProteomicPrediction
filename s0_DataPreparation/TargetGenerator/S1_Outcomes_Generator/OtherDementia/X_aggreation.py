...
dpath = '/Volumes/JasonWork/Projects/AD_Proteomics/Data/TargetOutcomes/OtherDementia/'
alg_df = pd.read_csv(dpath + 'OD_ALG.csv', usecols = ['eid', 'target_y', 'BL2Target_yrs', 'Target_source'])
alg_df.rename(columns = {'target_y':'alg_y', 'BL2Target_yrs':'alg_yrs', 'Target_source':'alg_source'}, inplace = True)

fo_df = pd.read_csv(dpath + 'OD_FO.csv', usecols = ['eid', 'target_y', 'BL2Target_yrs', 'Target_source'])
fo_df.rename(columns = {'target_y':'fo_y', 'BL2Target_yrs':'fo_yrs', 'Target_source':'fo_source'}, inplace = True)

death_df = pd.read_csv(dpath + 'OD_Death.csv', usecols = ['eid', 'target_y', 'BL2Target_yrs'])
death_df.rename(columns = {'target_y':'death_y', 'BL2Target_yrs':'death_yrs'}, inplace = True)

inhosp_df = pd.read_csv(dpath + 'OD_InHosp.csv', usecols = ['eid', 'target_y', 'BL2Target_yrs'])
...
rm_combid_idx1 = mydf.index[(mydf.target_y == 1) & (mydf.ad_y == 1)]
rm_combid_idx2 = mydf.index[(mydf.target_y == 1) & (mydf.vd_y == 1)]
rm_combid_idx = list(set(rm_combid_idx1.tolist() + rm_combid_idx2.tolist()))
mydf.drop(rm_combid_idx, axis = 0, inplace = True)
mydf.reset_index(inplace = True, drop = True)

mydf.to_csv(dpath + 'OD_outcomes.csv', index = False)
