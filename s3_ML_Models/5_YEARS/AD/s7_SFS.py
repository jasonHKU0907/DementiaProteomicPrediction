...
def get_top_pros(pro_f_df):
    i = 0
    auc_lst = pro_f_df.AUC_mean.tolist()
    while (auc_lst[i+2] - auc_lst[i] > 0.01):
        i += 1
    return i+1
...
outfile = outpath + 'ML_Modeling/5_YEARS/AD/s7_SeqSelector.csv'

pro_f_df = pd.read_csv(outpath + 'ML/5_YEARS/AD/AccAUC_TotalGain.csv')
nb_top_pros = 7
...
target_df = pd.read_csv(dpath + 'TargetOutcomes/AD/AD_outcomes.csv', usecols = ['eid', 'target_y', 'BL2Target_yrs'])
...
f_rank_df = pd.read_csv(outpath + 'ML_Modeling/5_YEARS/AD/s6_FeaImpRank.csv')
